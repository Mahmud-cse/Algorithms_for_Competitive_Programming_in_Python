...
####example code : create a latex graph with a minimum spanning tree
...
if __name__ == "__main__":
    ...
    with open('../Data/kruskal_latex/kruskal-points.tex', 'w') as infile:
        infile.write('\\documentclass{article}')
        infile.write('\\usepackage{tikz}')
        infile.write('\\begin{document}')
        infile.write('\\begin{tikzpicture}')
        ...
        for point in points:
            infile.write('\\filldraw[orange] (%f, %f) circle (1pt);\n'
                         % tuple(point))
        infile.write('\\end{tikzpicture}')
